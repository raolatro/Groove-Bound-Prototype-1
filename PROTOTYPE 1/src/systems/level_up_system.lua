...
-- Create a fallback shop module in case the real one fails to load
local LevelUpShop
pcall(function() 
    LevelUpShop = require("src.ui.level_up_shop")
end)

-- If LevelUpShop is still nil or not a table, create a fallback
if not LevelUpShop or type(LevelUpShop) ~= "table" then
    Debug.log("WARNING: Creating fallback LevelUpShop module")
    LevelUpShop = {
        init = function(self, player, levelupSystem)
            self.player = player
            self.levelUpSystem = levelupSystem
            return self
        end,
        open = function(self, player, weaponSystem, passiveSystem) 
            Debug.log("FALLBACK: LevelUpShop:open called")
            return self
        end,
...
function LevelUpSystem:init(weaponSystem, passiveSystem, player)
    ...
    local ok, result = pcall(function()
        local ShopModule = require("src.ui.level_up_shop")
        Debug.log("LevelUpShop module loaded successfully")
        return ShopModule:init(player, self)
    end)
    ...
    if ok then
        ...
        Debug.log("ERROR initializing shop: " .. tostring(result))
        -- Create a minimal fallback shop if loading fails
        self.shop = {
            init = function() return {} end,
            open = function() 
                Debug.log("FALLBACK SHOP: Open called - normal shop failed to load")
                Event.dispatch("LEVEL_UP_SHOP_CLOSED", {}) -- Auto-close immediately
            end,
            close = function() end,
            update = function() end,
            draw = function() end,
            isOpen = false,
            weaponSystem = self.weaponSystem,
            passiveSystem = self.passiveSystem
        }
    end
    
    -- Always set system references regardless of shop implementation
    -- This ensures any custom or fallback shop has the necessary references
    self.shop.weaponSystem = self.weaponSystem
    self.shop.passiveSystem = self.passiveSystem
    self.shop.player = self.player
    self.shop.levelUpSystem = self
    
    -- Log the status of all systems
    Debug.log("LevelUpSystem shop references:")
    Debug.log("  weaponSystem: " .. (self.weaponSystem and "OK" or "MISSING"))
    Debug.log("  passiveSystem: " .. (self.passiveSystem and "OK" or "MISSING"))
    Debug.log("  player: " .. (self.player and "OK" or "MISSING"))
    
    -- Fail-safe check - if shop has an explicit setup method, call it too
    if type(self.shop.setupSystemReferences) == "function" then
        self.shop:setupSystemReferences(self.player, self.weaponSystem, self.passiveSystem)  
    end
...
function LevelUpSystem:triggerLevelUp()
    ...
    Debug.log("PLAYER LEVEL UP: Level " .. self.currentLevel .. 
              " with " .. self.currentXP .. "/" .. self.xpToNextLevel .. " XP")
    
    -- Open the level-up shop UI
    self:openShop()
end

-- Open the level-up shop UI
function LevelUpSystem:openShop()
    -- Skip if already leveling up
    if self.isLevelingUp then
        Debug.log("LevelUpSystem: Cannot open shop, already leveling up")
        return
    end
    
    -- CRITICAL SEQUENCE: First pause the game, then open shop
    
    -- 1. Flag that we're leveling up
    ...
    -- The shop will open after this delay
    local pauseDelay = 0.05 -- 50ms delay for pause to take effect
    self.shop.openDelay = pauseDelay
    
    -- Double-check system references before passing them to shop
    Debug.log("SHOP OPEN: Checking system references")
    if not self.player then
        Debug.log("ERROR: Missing player reference before shop open")
    end
    if not self.weaponSystem then
        Debug.log("ERROR: Missing weaponSystem reference before shop open")
    end
    if not self.passiveSystem then
        Debug.log("ERROR: Missing passiveSystem reference before shop open")
    end
    
    -- Ensure shop has all references
    self.shop.player = self.player
    self.shop.weaponSystem = self.weaponSystem
    self.shop.passiveSystem = self.passiveSystem
    
    -- 4. Tell shop to open but wait for the delay (protect with pcall for safety)
    local ok, err = pcall(function()
        self.shop:open(self.player, self.weaponSystem, self.passiveSystem)
    end)
    
    if not ok then
        Debug.log("ERROR opening shop: " .. tostring(err))
        -- Auto-close in case of error
        self:closeShop()
    else
        Debug.log("LevelUpSystem: Opened level-up shop with " .. pauseDelay .. "s delay")
    end
    ...
    Debug.log("LevelUpSystem: Level-up shop opened")
